...
def build_instruction(base: str = "") -> str:
    ...
    return (
        ...
        f"{_calendar(now)}\n"
        f"Do not call book_meeting until you have the visitor's name, email AND a "
        f"specific time. If they give only a day, or a vague part of it "
        f"(\"morning\", \"afternoon\", \"sometime tomorrow\"), ask which time - "
        f"he is free 9am, 12pm, 2pm or 4:30pm IST - and book only once they pick. "
        f"Never book a time the visitor did not say.\n"
